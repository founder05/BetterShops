 ...
 * ************************************************************************
 */
public class MainSelling implements ShopMenu {

    Shop shop;
    Inventory inv;

    public MainSelling(Shop shop) {
        this.shop = shop;
                    ...
                    if (it.getPage() == page) {
                        ...
                        if (!it.getLiveEco()) {
                            ...
                            if (it.getAdjustedPrice() != it.getOrigPrice()) {
                                lore.add(Language.getString("MainGUI", "AskingPrice") + " §c§m" + it.getOrigPrice() + " §a" + it.getAdjustedPriceAsString());
                            } else {
                                if (it.getPrice() > 0) {
                                    lore.add(Language.getString("MainGUI", "Price") + " §7" + it.getPriceAsString());
                                } else {
                        ...
                        itemClick.addLeftClickAction(new LeftClickAction() {
                            @Override
                            public void onAction(InventoryClickEvent e) {
                                shop.getMenu(MenuType.SELL_ITEM).draw(p, page, itemStack, it);
                            }
                        });
                    }
                }
