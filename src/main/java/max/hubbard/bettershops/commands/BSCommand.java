...
import max.hubbard.bettershops.configuration.Blacklist;
import max.hubbard.bettershops.configuration.Config;
import max.hubbard.bettershops.configuration.ConfigMenu.ConfigMenu;
import max.hubbard.bettershops.configuration.Language;
import max.hubbard.bettershops.configuration.LanguageMenu.GUIMessagesInv;
import max.hubbard.bettershops.configuration.Permissions;
import max.hubbard.bettershops.listeners.Opener;
import max.hubbard.bettershops.nms.SignChange;
            ...
            if (sender instanceof Player) {
                ...
                if (args.length == 1) {
                    ...
                    if (args[0].equalsIgnoreCase("migrate") && p.isOp()) {
                        ...
                        p.sendMessage(
                            Language.getString("Messages", "Prefix") + "You are running version: §e"
                                + Bukkit.getPluginManager().getPlugin("BetterShops")
                                .getDescription().getVersion());

                        p.sendMessage(Language.getString("Messages", "Prefix") + "Total shops: §e"
                            + ShopManager.getShops().size());
                        p.sendMessage(
                            Language.getString("Messages", "Prefix") + "Total NPC shops: §e"
                                + NPCManager.getNPCShops().size());
                        p.sendMessage(
                            Language.getString("Messages", "Prefix") + "Total Holographic shops: §e"
                                + HologramManager.getHolographicShops().size());
                        p.sendMessage(
                            Language.getString("Messages", "Prefix") + "Total Sign shops: §e"
                                + SignShopManager.getSigns().size());
                    ...
                    if (args[0].equalsIgnoreCase("move")) {
                        if ((boolean) Config.getObject("Permissions") && Permissions.hasMoveCommandPerm(p) || !(boolean) Config.getObject("Permissions")) {
                            ...
                            Shop shop = ShopManager.fromString(p, name.toString());
                        ...
                        if ((boolean) Config.getObject("Permissions") && Permissions.hasListPerm(p) || !(boolean) Config.getObject("Permissions")) {
                            ...
                            if (pl != null) {
                                ...
                                if (shops != null && shops.size() > 0) {
                                    ...
                                    p.sendMessage(
                                        "§d<-Listing §e" + pl.getName() + "'s §dShops (§c0§d)->");
                                                            ...
                                                            if (face.getType() == Material.CHEST || face.getType() == Material.TRAPPED_CHEST) {
                                                                if (face.getState() instanceof Chest) {
                                                                    ...
                                                                    if (ch.getLocation().equals(loc)) {
                                                                        ...
                                                                        sign.getWorld().dropItem(loc, new ItemStack(
                                                        ...
                                                        if (bs instanceof Sign) {
                                                            ...
                                                            if (face.getType() == Material.CHEST || face.getType() == Material.TRAPPED_CHEST) {
                                                                if (face.getState() instanceof Chest) {
                                                                    ...
                                                                    if (ch.getLocation().equals(loc)) {
                                                                        sign.getBlock().setType(Material.AIR);
                                                                        sign.getWorld().dropItem(loc, new ItemStack(XMaterial.OAK_SIGN.parseMaterial()));
                                                                    }
                                                                }
                                                            }
                                                        }
