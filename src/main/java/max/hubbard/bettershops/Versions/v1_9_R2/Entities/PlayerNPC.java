...
import max.hubbard.bettershops.Versions.v1_9_R2.NPCConnection;
...
public class PlayerNPC extends EntityPlayer {
    ...
    public PlayerNPC(MinecraftServer minecraftserver, WorldServer worldserver,
        GameProfile gameprofile, PlayerInteractManager playerinteractmanager) {
    ...
    public static Player spawn(GameProfile pro, Location location) {
        ...
        final PlayerNPC customEntity =
            new PlayerNPC(server, world, pro, new PlayerInteractManager(world));
        customEntity
            .setLocation(location.getX(), location.getY(), location.getZ(), location.getYaw(),
                location.getPitch());
        customEntity.getBukkitEntity().setRemoveWhenFarAway(false);
