...
public class ShopDeleter {
    ...
    public static void deleteShopExternally(final Shop shop) {
        ...
        if (shop instanceof FileShop) {
            ...
            try {
                ((SQLShop) shop).statement.executeUpdate(
                    "DELETE FROM " + Config.getObject("prefix") + "shops WHERE Name = '" + shop
                        .getName() + "';");
                ((SQLShop) shop).statement.executeUpdate(
                    "DELETE FROM " + Config.getObject("prefix") + "Trades WHERE Shop = '" + shop
                        .getName() + "';");
