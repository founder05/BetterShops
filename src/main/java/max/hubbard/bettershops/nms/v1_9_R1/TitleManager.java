 ...
 * ************************************************************************
 */
public class TitleManager implements max.hubbard.bettershops.nms.TitleManager {

    public void sendTitle(Player p, String message) {

        PacketPlayOutTitle title = new PacketPlayOutTitle(PacketPlayOutTitle.EnumTitleAction.TITLE,
            new ChatComponentText(
                ChatColor.translateAlternateColorCodes("§".toCharArray()[0], message)));
    ...
    public void setTimes(Player p, int fadein, int amt, int fadeout) {
        ...
        ((CraftPlayer) p).getHandle().playerConnection
            .sendPacket(new PacketPlayOutTitle(fadein, amt, fadeout));
